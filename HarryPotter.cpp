...
//Default constructor
HarryPotter::HarryPotter(std::string charName):Character(charName)
{
  ...
  //Bool variable to use for harry's special ability.
  hogwarts = true;

  this->charName =  this->charName + " HarryPotter";
}
...
{
  //Harry potter rolls two die, each with six sides.
  int attack = roll(2, 6);
...
{
  //Defense die roll for harry is two die with six sides.
  int defense = roll(2, 6);
  ...
  ***********************************************************************/
  ...
  //Function for harry potter special ability. True until used.
  if(strength == 0 && hogwarts)
  {
    //Set strength/health to twenty.
    strength = 20;

    //Print to screen 6. The defender’s updated strength point amount.
    cout << charName << " Hogwarts ability used. Harry recovers to 20 strength." << endl;

    //Set bool variable to false, in order to Not use ability again.
    hogwarts = false;
  }
